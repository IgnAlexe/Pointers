...
template<typename T>T** insert_row(T** arr, int& rows, const int cols);
template<typename T>T** erase_row(T** arr, int& rows, const int cols);
template<typename T>void insert_col(T** arr, const int rows, int& cols);
template<typename T>void erase_col(T** arr, const int rows, int& cols);
...
void main()
{
	...
	double** arr = Allocate(rows, cols);					//Обьявляем динамический двумерный массив	
